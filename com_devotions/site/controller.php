...
class DevotionsController extends JController {
    ...
    function add_partner() {
        ...
        $fp = $this->getPastorId($user->id);
        $sp = JRequest::getVar('sp', '', 'get', 'int');

        if(empty($fp) || empty($sp)) {
            $mainframe->redirect($refer, 'Your request contains errors!', 'error');
            exit();
        }
        
        if($fp == $sp) {
            $mainframe->redirect($refer, 'You cannot send a request to yourself.', 'error');
            exit();
        }
        
        $sucess = $model->sendRequest($fp, $sp, 0);
        
        if (!$sucess) {
            $mainframe->redirect($refer, 'Error, request not sent!', 'error');
            exit();
        }

        $mainframe->redirect($refer, 'Your request has been sent!');
    }
    ...
    function handle_request() {
        $mainframe =& JFactory::getApplication();
        $model =& $this->getModel('divinecall');
        
        $refer = JRoute::_($_SERVER['HTTP_REFERER']);
        
        $id = JRequest::getVar('id', '', 'get', 'int');
        $res = JRequest::getVar('res', '', 'get', 'string');
        ...
        $sucess = $model->handleRequest($res, $id);
        
        if (!$sucess) {
            $mainframe->redirect($refer, 'Database Error!', 'error');
            exit();
        }

        $mainframe->redirect($refer, $sucess);
    }
    ...
    function search() {
        ...
        $query = "INSERT INTO #__devotion_parners () VALUES()";
        $db->setQuery($query);
        $pid = $db->loadResult();        
